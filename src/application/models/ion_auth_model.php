...
class Ion_auth_model extends CI_Model
{
    ...
    public function change_password($identity, $old, $new)
    {
        ...
        if ($old_password_matches === true) {
            ...
            $successfully_changed_password_in_db = $this->db->update(
                $this->tables['users'],
                $data,
    ...
    public function forgotten_password($identity)
    {
        ...
        if ($key != '' && $this->config->item('permitted_uri_chars') != '' && $this->config->item(
    ...
    public function forgotten_password_complete($code, $salt = false)
    {
        ...
        if (empty($code)) {
            $this->trigger_events(
                array('post_forgotten_password_complete', 'post_forgotten_password_complete_unsuccessful')
            );
        ...
        if ($profile) {
            ...
            $this->trigger_events(
                array('post_forgotten_password_complete', 'post_forgotten_password_complete_successful')
            );
    ...
    public function register($username, $password, $email, $additional_data = array(), $groups = array())
    {
        ...
        $query = $this->db->get_where(
            'groups',
            array('name' => $this->config->item('default_group', 'ion_auth')),
            1
        )->row();
    ...
    public function is_time_locked_out($identity)
    {
        ...
        return $this->is_max_login_attempts_exceeded($identity) && $this->get_last_attempt_time($identity) > time(
    ...
    public function increase_login_attempts($identity)
    {
        if ($this->config->item('track_login_attempts', 'ion_auth')) {
            ...
            return $this->db->insert(
                $this->tables['login_attempts'],
                array('ip_address' => $ip_address, 'login' => $identity, 'time' => time())
            );
        }
    ...
    public function get_users_groups($id = false)
    {
        ...
        return $this->db->select(
            $this->tables['users_groups'] . '.' . $this->join['groups'] . ' as id, ' . $this->tables['groups'] . '.name, ' . $this->tables['groups'] . '.description'
        )
    ...
    public function add_to_group($group_ids, $user_id = false)
    {
        ...
        foreach ($group_ids as $group_id) {
            if ($this->db->insert(
                $this->tables['users_groups'],
                array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id)
            )
            ) {
    ...
    public function remove_from_group($group_ids = false, $user_id = false)
    {
        ...
        if (!empty($group_ids)) {
            ...
            if ($return = $this->db->delete(
                $this->tables['users_groups'],
                array($this->join['users'] => (int)$user_id)
            )
            ) {
    ...
    public function update($id, array $data)
    {
        ...
        if (array_key_exists($this->identity_column, $data) && $this->identity_check(
    ...
    {
        if (isset($this->_ion_hooks->{$event}[$name]) && method_exists(
